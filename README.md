...
### 단순 청킹 vs LLM 전처리 — 뭐가 다른가?

일반적인 RAG 파이프라인은 문서를 **고정 길이로 잘라서**(500자, 1000자 등) 임베딩한다.
이 방식은 빠르고 비용이 0원이지만, 문맥이 잘리거나 관련 없는 내용이 한 청크에 섞이는 문제가 있다.

```
예) 고정 500자 청킹:
"...CNN은 이미지 인식에 혁명을 가져왔고, RNN과 LSTM은 자연어 처리와 시계열"  ← 여기서 잘림
"데이터에 활용되었다. 2017년 Transformer 아키텍처의 등장으로..."            ← 문맥 단절
```

LLM 전처리는 **문서를 읽고 의미 단위로 나눈다**:
```
예) LLM 의미 청킹:
청크 1: "머신러닝의 개념" — 지도학습, 비지도학습, 강화학습 전체 설명
청크 2: "딥러닝의 발전"  — CNN, RNN, Transformer까지 하나의 흐름
```

| | 단순 청킹 (RecursiveTextSplitter 등) | LLM 전처리 (이 파이프라인) |
|---|---|---|
| **청크 품질** | 문맥이 잘릴 수 있음 | 의미 단위로 깔끔하게 분할 |
| **메타데이터** | 없음 (직접 추가해야 함) | title, topic, summary, keywords 자동 추출 |
| **검색 필터링** | 벡터 유사도만 의존 | 토픽/키워드로 사전 필터 후 벡터 검색 가능 |
| **비용** | 0원 | 문서당 LLM API 호출 비용 발생 |
| **속도** | 즉시 | 문서당 수 초 (API 호출) |
| **대규모 처리** | 수만 건도 OK | 수백 건 수준에 적합 |

### 그래서 언제 이걸 쓰면 좋나?

**이 파이프라인이 효과적인 경우:**
- 문서가 수십~수백 개 수준이고, 검색 품질이 중요할 때
- 비정형 텍스트가 많아서 단순 자르기로는 품질이 안 나올 때
- 토픽/키워드 기반 필터링이 필요할 때 (예: "인공지능 관련 문서에서만 검색")

**단순 청킹이 더 나은 경우:**
- 문서가 수만 건 이상 — LLM 비용과 시간이 부담
- 이미 잘 구조화된 문서 (마크다운, HTML 등) — LLM이 할 일이 별로 없음
- 빠른 프로토타이핑이 목표일 때

**실전 팁 — 하이브리드 전략:**
> 신규 문서(소량)는 LLM 전처리로 고품질 청크를 만들고,
> 대량의 기존 문서는 단순 청킹 + 좋은 임베딩 모델로 처리.
> 검색 시 hybrid search(벡터 + 키워드 BM25)를 쓰면 두 방식의 장점을 모두 취할 수 있다.
...
# 특정 시각 이후 변경된 파일만 실행
python pipeline.py --since 1709553600.0
```

**동시 실행 방지:** `fcntl.flock` 기반 파일 락 사용. 이미 실행 중이면 `PipelineBusy` 예외 발생.
...
- **실행 모드 선택**: "변경된 파일만 (N개)" vs "전체 재실행 (N개)"
  - 변경된 파일만: 마지막 실행 이후 새로 추가/수정된 파일만 처리
  - 전체 재실행: archive의 기처리 파일까지 다시 가져와서 전부 처리
- **동시 실행 방지**: 누군가 파이프라인 실행 중이면 🔒 표시 + 버튼 비활성화

#### 업로드 탭 상세

- **파일 업로드**: .txt 드래그 앤 드롭 (다중 선택 가능), 동명 파일은 타임스탬프 접미사
- **대기 파일 목록**: 🆕 새 파일 / 📄 기존 파일 구분, 제목 + 300자 미리보기
- **파일 편집기**: input_docs + archive_docs 파일 선택 → 브라우저에서 직접 조회/수정/새 파일 생성

#### 멀티유저 지원

- Streamlit 세션 격리 내장 (브라우저 탭마다 독립)
- `fcntl.flock` 기반 파이프라인 동시 실행 방지
- LAN 접속: `--server.address 0.0.0.0` 옵션

#### 긴 문서 처리 시 주의

- Gemini Flash 기준 **10KB 이하** 문서 권장
- 10KB 이상은 JSON 응답이 깨질 수 있음 → 챕터 단위로 분할 후 투입
- `llm.py`에 JSON 파싱 실패 시 1회 자동 재시도 로직 포함
